...
interface VehicleRow {
  v: Vehicle
  rev: number
  fuelIn: number    // น้ำมันในโรงงาน (station === 'ถังโรงงาน')
  fuelOut: number   // น้ำมันนอก (external stations + dispatch fuel)
  allowance: number
  salary: number
  expense: number   // ค่าใช้จ่าย = ซ่อม + ยาง + misc
  interest: number  // ดอกเบี้ย (FixedCost category === 'ดอกเบี้ย')
  totalCost: number
  profit: number
}
...
function computeRows(
  ...
  return vehicles.map(v => {
    ...
    const fuelOutLogs = myFuelLogs
      .filter(f => f.station !== 'ถังโรงงาน')
      .reduce((s, f) => s + (f.total ?? 0), 0)
    // Dispatch fuel cost (d.cost) treated as external — pumped during trips
    const dispatchFuel = myDispatches.reduce((s, d) => s + (d.cost ?? 0), 0)
    const fuelOut = fuelOutLogs + dispatchFuel
    ...
    // ดอกเบี้ย — only fixed costs categorised as 'ดอกเบี้ย' for this vehicle
    ...
    return {
      v, rev, fuelIn, fuelOut, allowance, salary,
...
function MetricCard({
  ...
  const bg = tone === 'green' ? '#DCFCE7' : tone === 'red' ? '#FEE2E2' : tone === 'amber' ? '#FEF3C7' : '#EFF6FF'
  return (
    <div className="card" style={{ padding: '18px 20px', background: '#ffffff' }}>
      <div className="row" style={{ gap: 14, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className="mono" style={{ fontSize: 22, fontWeight: 700, color, letterSpacing: '-.01em' }}>
            {value}
...
function InterestCell({
  ...
  return (
    <div
      className="row"
      style={{ gap: 4, justifyContent: 'flex-end', cursor: 'pointer', minWidth: 90 }}
      onClick={() => setEditing(true)}
      title="คลิกเพื่อแก้ไขดอกเบี้ย"
    >
...
export function FinancePL() {
  ...
  const [tick, setTick] = useState(0) // increment to force re-aggregate after interest edit

  const ym = `${year}-${String(month + 1).padStart(2, '0')}`

  // Year range พ.ศ. 2568–2578 (CE 2025–2035)
  const yearOptions = useMemo(() =>
    Array.from({ length: 11 }, (_, i) => 2025 + i),
    [],
  )

  const { rows, totals, vehicleCount } = useMemo(() => {
    try {
      ...
      const rs = computeRows(vehicles, dispatches, fuel, maint, expenses, fixedCosts, employees, ym)
      const t = rs.reduce(
        (acc, r) => ({
          rev: acc.rev + r.rev,
        ...
        { rev: 0, fuelIn: 0, fuelOut: 0, allowance: 0, salary: 0, expense: 0, interest: 0, totalCost: 0, profit: 0 },
      )
      return { rows: rs, totals: t, vehicleCount: vehicles.length }
    } catch (err) {
      console.error('FinancePL aggregation failed', err)
      return {
        rows: [],
        vehicleCount: 0,
        totals: { rev: 0, fuelIn: 0, fuelOut: 0, allowance: 0, salary: 0, expense: 0, interest: 0, totalCost: 0, profit: 0 },
      }
    }
  }, [ym, tick])

  const isProfit = totals.profit >= 0
  const activeRows = rows.filter(r => r.rev > 0 || r.totalCost > 0)
    ...
    <div>
      ...
      >
        <MetricCard
          ...
          tone="blue"
          icon="money"
          subtitle={`${activeRows.length} คันที่มีรายการ`}
        />
        <MetricCard
          label="รายจ่ายรวม"
      ...
      {/* ── Filter bar ── */}
      <div className="card no-print" style={{ padding: '14px 18px', marginBottom: 16, background: '#ffffff' }}>
      ...
      <div className="card print-area" style={{ background: '#ffffff' }}>
        <div className="head no-print" style={{ paddingBottom: 0 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginLeft: 'auto' }}>
            <Icon name="edit" size={11} /> คลิกที่คอลัมน์ "ดอกเบี้ย" เพื่อกรอกข้อมูล
          </div>
        </div>
        ...
        {rows.length === 0 ? (
          ...
          <div className="tbl-wrap" style={{ border: 'none', borderRadius: 0 }}>
            <table className="tbl">
              <thead>
                <tr>
                  <th style={{ whiteSpace: 'nowrap' }}>ทะเบียนรถ</th>
              ...
              <tbody>
                {rows.map(r => {
                  ...
                  return (
                    <tr
                      key={r.v.id}
                      style={{ opacity: hasActivity ? 1 : 0.55 }}
                    >
                      <td>
      ...
      <div className="no-print" style={{
        ...
        <span>
          คอลัมน์ <strong>ดอกเบี้ย</strong>: คลิกที่ตัวเลขในตารางเพื่อกรอกหรือแก้ไขดอกเบี้ยรายคัน
          · ข้อมูลจะบันทึกลงระบบโดยไม่กระทบค่าใช้จ่ายหมวดอื่น
          · สามารถดูรายการทั้งหมดได้ที่ <strong>การเงิน → ค่าใช้จ่ายคงที่</strong>
        </span>
      </div>
