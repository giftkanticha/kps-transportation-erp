...
function LoginScreen({ onLogin }) {
  ...
  const seedUsers = window.KPSData.getAll("users");

  const submit = (e) => {
    e && e.preventDefault();
    if (!email) { setErr("กรุณากรอกอีเมล"); return; }
    const u = window.KPSData.login(email);
  ...
  const roleLabel = (r) => r === "admin" ? "Administrator" : r === "manager" ? "Manager" : "Driver / Employee";
    ...
    <div className="login-wrap">
      <div className="login-poster">
        <div className="brand-big">
          ...
          <div>
            <div style={{ fontSize: 13, opacity: .7, letterSpacing: ".08em", textTransform: "uppercase" }}>KPS Logistics</div>
      ...
      <div className="login-form">
        ...
        <form onSubmit={submit} style={{ marginTop: 28, display: "flex", flexDirection: "column", gap: 14 }}>
          <Field label="อีเมล / Email">
            <input value={email} onChange={e => { setEmail(e.target.value); setErr(""); }} placeholder="you@kps.com" autoFocus/>
          </Field>
          ...
          <button type="submit" className="btn primary" style={{ height: 38, justifyContent: "center", marginTop: 6 }}>
            ...
            {seedUsers.map(u => (
              <div
                ...
                style={{
                  ...
                  border: picked === u.id ? "1px solid var(--navy-600)" : "1px solid transparent",
                  cursor: "pointer",
                }}
              >
